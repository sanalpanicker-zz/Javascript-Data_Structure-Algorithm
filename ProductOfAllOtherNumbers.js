...
let demoArr = [1, 2, 3, 4, 5, 6, 7, 8];
//[1, 1 , 2*1, 3*2*1, 4*3*2*1,.... ];

//all the previous number products

//tip get the previous numbers, get the after numbers and then multiple both 
const prevProduct = (arr) => {
    const beforeResultArr = [];
    let bproduct = 1;
    for (let i = 0; i < arr.length; i++) {
        beforeResultArr[i] = bproduct;
        bproduct = bproduct * arr[i];


    }

    //get after number product

    //     const afterResultArr = [];let aproduct =1;
    //   for(let i= arr.length -1 ; i >= 0; i--){
    //      afterResultArr[i] =aproduct;
    //     aproduct = aproduct * arr[i];

    //   }

    let aproduct = 1;
    for (let i = arr.length - 1; i >= 0; i--) {
        beforeResultArr[i] = beforeResultArr[i] * aproduct;
        aproduct = aproduct * arr[i];

    }
    console.log(demoArr);
    console.log(beforeResultArr);
}
