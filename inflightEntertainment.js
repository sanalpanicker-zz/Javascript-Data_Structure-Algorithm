...
let flightFillMovieLength = (movielengths, flightlength) => {
    let AvailableMovieOptions = [];
    let movieLengthSeen = {};

    movielengths.forEach((movielength, i) => {

        let secondMovieLength = flightlength - movielength;

        if (movieLengthSeen.hasOwnProperty(secondMovieLength)) {
            console.log(movieLengthSeen);
            AvailableMovieOptions.push([movielength, secondMovieLength]);
            //moviesAvailable = true;
        } else {
            movieLengthSeen[movielength] = i + 1;

        }
    });

    return AvailableMovieOptions;
};

console.log(flightFillMovieLength([6, 4, 3, 2, 1, 5, 7], 10));
